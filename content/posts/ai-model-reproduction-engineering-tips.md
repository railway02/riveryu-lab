...
适合**调试流程、验证代码、跑 baseline**。如果要做正式实验、定量指标或者论文结果，仍然应该使用真实的预处理数据，否则实验结论可能不严谨。

更准确地说，是**使用占位输入进行流程验证**。
...
2. **输入格式要对齐**：很多报错是文件名、目录、尺寸、后缀、帧序号不符合要求。
