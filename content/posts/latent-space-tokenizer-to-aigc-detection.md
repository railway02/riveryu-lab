...
RAE latent 信息更多，但计算和显存压力更大。
尤其是做视频时，如果每帧都使用高维 RAE latent，时序长度一增加，成本会迅速上升。
...
RAE 不能简单替换 VAE。
需要调整 DiT width、噪声调度、head 结构和 decoder 训练方式。
...
原始 RAE 使用强语义 encoder 的最后层特征。
最后层特征语义强，但局部空间细节可能不足。

对于图像生成，这可能影响纹理细节。
对于视频生成，这可能导致帧间局部细节不稳定。
...
真实视频的 representation 轨迹通常更符合物理连续性。
生成视频可能出现局部跳变、过度平滑、身份漂移或背景 representation 抖动。
...
## 30. 一个更强的检测框架：双潜空间数字指纹

可以设计一个框架：

```text
Dual-Space Latent Fingerprint for AIGC Video Detection
```

中文可以叫：

```text
面向 AIGC 视频检测的双潜空间数字指纹方法
```
